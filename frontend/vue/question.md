...
- [Vue Question](#vue-question)
  - [Question](#question)
    ...
    - [解決方法](#解決方法)
      - [範例](#範例)
    - [Q. computed vs method vs watch](#q-computed-vs-method-vs-watch)
      - [Computed](#computed)
      - [Method](#method)
      - [Watch](#watch)
...
### Q. computed vs method vs watch

| 特性     | Computed               | Method                 | Watch          |
| -------- | ---------------------- | ---------------------- | -------------- |
| 依賴值   | 有                     | 無                     | 手動設置       |
| 快取     | 有                     | 無                     | 無             |
| 使用時機 | 需要依賴值計算出結果時 | 需要執行較複雜的邏輯時 | 監聽資料變化時 |
...
- 重點
  ...
  - 依據多個依賴值變化去計算一個結果
- 定義:
  - Computed 屬於一種“計算型屬性”，會**根據 Vue 實例數據的變化而變化**，當依賴的數據發生變化時，自動重新計算相應的屬性值，可以緩存計算結果，當多個地方需要用到計算結果時，可以減少計算量。
  - Computed 屬性的結果**緩存**且只在必要時重新計算，這意味著只有在其相依的屬性發生變化時，才會重新計算值。
  - Computed 屬性通常用於需要對已有屬性進行一些計算的場景。
- 舉例:
  - 計算購物車內所有商品的總價。
  - 根據當前選中的地址計算運費。
...
- 重點
  ...
  - 無快取特性
- 定義:
  - Methods 可以被當作 Vue 實例的方法來使用，在模板中用方法名進行調用。
  - 每次調用方法時都會重新運行一次，不進行緩存，對於需要每次都重新計算的情況很有用。
  - Methods 通常用於事件處理或特殊操作的場景。
- 舉例:
  - 點擊購物車中的“加入收藏”按鈕，將商品加入收藏夾。
  - 點擊購物車中的“去結算”按鈕，跳轉到結算頁面。
...
- 重點
  - 可支援非同步場景 (搜尋框)
- 定義:
  - Watch 是監聽數據變化的選項，可以為數據的變化執行自定義邏輯。
  - Watch 的值可以是數據源的字符串形式，也可以是一個函數。
  - Watch 屬性主要用於需要對數據進行非同步或複雜操作的場景。
- 舉例:
  - 監聽購物車內商品數量的變化，實時更新購物車總價。
  - 監聽用戶登錄狀態的變化，當用戶登錄後自動將購物車中的商品添加到用戶的訂單中。
